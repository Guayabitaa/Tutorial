package dev.tutorial.utilities;
...
public class CooldownUtil {
    ...
    public void setCooldown(String name, Player player, long time) {
        if (time == 0L) cooldown.remove(name, player.getUniqueId());
        if (time <= 0L) cooldown.remove(name, player.getUniqueId());
        else cooldown.put(name, player.getUniqueId(), System.currentTimeMillis() + time);
    }
